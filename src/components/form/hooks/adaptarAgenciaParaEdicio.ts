...
export function adaptarAgenciaParaEdicion(agencia: AgenciaBackData): {
  ...
  const valores: AgenciaFormValues = {
    ...
    dominio: (agencia as any).dominio ?? '', // Fallback si no viene
    estado: agencia.estado,
    password: '', // Nunca precargar por seguridad
    ...
    buscador_inputColor: agencia.buscador.inputColor ?? undefined,
    buscador_inputFondoColor: agencia.buscador.inputFondoColor ?? undefined,
    ...
    // --- Publicidad Cliente ---
    publicidad_existe: agencia.publicidadCliente.existe,
  ...
  const urlsAgencia: UrlsAgencia = {
    ...
    terminosUrl: agencia.terminos_y_condiciones || agencia.terminosycondiciones,
  };
